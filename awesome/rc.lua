...
-- run_once({ "wezterm", "unclutter -root" }) -- comma-separated entries
...
globalkeys = mytable.join(
    ...
    -- ALSA volume control
    awful.key({ }, "XF86AudioRaiseVolume", -- Fn + Up
        function ()
            os.execute(string.format("amixer -q set %s 1%%+", beautiful.volume.channel))
            beautiful.volume.update()
        end,
        {description = "volume up", group = "hotkeys"}),
    awful.key({ }, "XF86AudioLowerVolume", -- Fn + Down
        function ()
            os.execute(string.format("amixer -q set %s 1%%-", beautiful.volume.channel))
            beautiful.volume.update()
        end,
        {description = "volume down", group = "hotkeys"}),
    awful.key({  }, "XF86AudioMute", -- Fn + F8
        function ()
...
awful.rules.rules = {
    ...
    { rule_any = {type = { "normal", "dialog" }
      }, properties = { titlebars_enabled = false } -- Скрываем заголовки приложений
    },
